...
def step(client: AgentTraderClient, n: int) -> None:
    ...
    for sym, pos in held.items():
        ...
        if sym not in targets or drawdown <= -STOP_PCT:
            why = (
                f"stop-loss {drawdown:.2f}%"
                if drawdown <= -STOP_PCT
                else f"dropped out of top-{TOP_N} momentum"
            )
            try:
                o = client.sell(
                    sym, pos["quantity"], reasoning=why, client_order_id=f"exit-{sym}-{n}", tag="momentum"
                )
    ...
    for sym in targets:
        ...
        try:
            ...
            print(
                f"[{n}] BUY  {sym} x{qty} -> {o['status']} @ {o['average_price']} charges ₹{o['charges']} ({why})"
            )
        except AgentTraderError as e:
            print(f"[{n}] BUY {sym} refused: {e}")

    print(
        f"[{n}] {status['phase']} equity ₹{pf['equity']:,.2f} day P&L ₹{pf['day_pnl']:,.2f} positions {len(held)}"
    )
...
def run_demo(steps: int = 120) -> None:
    ...
    try:
        client = AgentTraderClient.register(
            server.base_url,
            name="momentum-demo",
            description="rule-based momentum example",
            admin_key="admin",
        )
...
if __name__ == "__main__":
    ...
    if a.url is None:
        ...
        c = (
            AgentTraderClient(a.url, api_key=a.api_key)
            if a.api_key
            else AgentTraderClient.register(a.url, name=a.name)
        )
